...
import pytest
from telegram.streaming.truncation import EFFECTIVE_LIMIT
...
class TestTruncationManager:
    ...
    def test_thinking_truncated_from_beginning(self):
        ...
        tm = TruncationManager()
        # Create content that exceeds EFFECTIVE_LIMIT (4046)
        # thinking = 4500 chars, text = 100 chars, total = 4600 > 4046
    ...
    def test_thinking_hidden_when_no_space(self):
        ...
        # Text = EFFECTIVE_LIMIT - 50 = 3996, thinking = 200
        # Total = 4196 > 4046, available for thinking = 4046 - 3996 - 1 = 49 < 100
    ...
    def test_calculate_available_space(self):
        ...
        assert tm.calculate_available_space(100) == EFFECTIVE_LIMIT - 100

        # Long text leaves less space
        assert tm.calculate_available_space(EFFECTIVE_LIMIT - 50) == 50

        # Very long text results in zero space
        assert tm.calculate_available_space(EFFECTIVE_LIMIT + 100) == 0
    ...
    def test_needs_truncation(self):
        ...
        assert not tm.needs_truncation(EFFECTIVE_LIMIT // 2,
                                       EFFECTIVE_LIMIT // 2)

        # Over limit - needs truncation
        assert tm.needs_truncation(EFFECTIVE_LIMIT, 100)

    def test_exact_limit_boundary(self):
        """Content exactly at limit is not truncated."""
        tm = TruncationManager()
        # Create content exactly at limit
        total_needed = EFFECTIVE_LIMIT
        text_len = 500
    ...
    def test_one_char_over_limit(self):
        ...
        total_needed = EFFECTIVE_LIMIT + 1
        text_len = 500
        thinking_len = total_needed - text_len
