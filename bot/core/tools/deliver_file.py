...
Use temp_id from execute_python/render_latex output_files.
Files cached 30 min — deliver promptly.

<modes>
- Default: parallel delivery, multiple files sent together
- sequential=True: turn break after delivery, allows text between files
- send_mode: "auto" (default), "photo" (compressed), "document" (full quality)
</modes>

Use sequential=True when explaining files one by one with text between them.
4K images auto-deliver as document to preserve quality.""",
    "input_schema": {
        "type": "object",
        "properties": {
            "temp_id": {
