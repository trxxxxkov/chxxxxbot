...
class GenerationTracker:
    ...
    Maintains a mapping of (chat_id, user_id) to asyncio.Event objects.
    ...
    Only one active generation per user per chat is tracked.
    Starting a new generation overwrites any previous one.

    Example:
        # In streaming handler:
        cancel_event = generation_tracker.start(chat_id, user_id)
        try:
            ...
            generation_tracker.cleanup(chat_id, user_id)

        # In callback handler:
        if generation_tracker.cancel(chat_id, user_id):
    ...
    async def start(self, chat_id: int, user_id: int) -> asyncio.Event:
        """Start tracking a new generation.

        Creates a new asyncio.Event for cancellation signaling.
        If a generation is already active for this user/chat, it's replaced.
    ...
    async def cancel(self, chat_id: int, user_id: int) -> bool:
    ...
    async def cleanup(self, chat_id: int, user_id: int) -> None:
...
async def generation_context(
    ...
    Example:
        async with generation_context(chat_id, user_id) as cancel_event:
            async for event in stream:
                if cancel_event.is_set():
                    break
                # process event...
    ...
    """
    # Start tracking
    cancel_event = await generation_tracker.start(chat_id, user_id)

    try:
        yield cancel_event
    finally:
        # Cleanup generation tracking
        await generation_tracker.cleanup(chat_id, user_id)
